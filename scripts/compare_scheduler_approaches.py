...
def extract_result(stdout, input_file):
    ...
    if "SAT found: Yes" in stdout or "SAT" in stdout and "UNSAT" not in stdout:
        result["status"] = "sat"
    elif "SAT found: No" in stdout or "UNSAT" in stdout:
        result["status"] = "unsat"
    ...
    if not time_match:
        time_match = re.search(
            r"schedule_calculation_seconds.*?([0-9.eE+-]+)",
            stdout,
            re.IGNORECASE,
        )
