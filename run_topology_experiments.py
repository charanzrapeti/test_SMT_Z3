...
def run_solver(input_file, timeout):
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate topology variants and optionally run test2.py for each one."
    )
    ...
    parser.add_argument(
        "--run-solver",
        action="store_true",
        help="Run test2.py after generating each topology.",
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    for topology in selected_topologies:
        ...
        if args.run_solver:
            print(f"Running solver for {topology}...")
            result.update(run_solver(output_file, args.timeout))
            if result["feasible"]:
